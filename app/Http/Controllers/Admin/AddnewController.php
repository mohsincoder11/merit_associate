...
//joins for dispalying data on onging tab in table
  ...
  ->leftjoin('employee_registraions as emp4','emp4.id','=','add_news.technical_head_id')
   ->select('add_news.*','locations.locations','associatesbanks.bankname','products.products','areas.area','emp1.name as field_executive',
        ...
        $status1 = Status:: where('role_name_id',Auth::user()->role_name_id)
        ->get();

        $status = Status ::
        when(Auth::user()->role_name_id = 25, function ($q)  { 
          //this condition will apply when user is not admin. admin has role 26
          return $q->where('role_name_id',Auth::user()->role_name_id);
      })
      ->get();
...
// exit();
// echo json_encode(auth()->user()->user_id);
// exit();
