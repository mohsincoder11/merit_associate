...
class LocationController extends Controller
{
    public function index()
    {
      ...
      ->select('areas.*','locations.locations')
        ...
        return view('Masters.location',compact('data','area_all'));
    }
   ...
   // echo json_encode($request->all());
      ...
      // exit();
    return view('Masters.edit_location',['edit_data'=>$data1,'data'=>$data,'area_all'=>$area_all]);
    }
...
//     }
    ...
    public function insert_area(request $request)
    {
       ...
       $store->save();
     //return response()->json(Area::select('id','area')->where('location_id',$request->get('location_id'))->orderby('area','asc')->get());
     return redirect(route('location'))->with(['success' => true, 'message' => 'Data Successfully Submitted !', 'location' => $request->get('location_id')]);

    }

    public function insert_area_ajax(request $request)
    {
       $store= new Area;
       $store->location_id=$request->get('location_id');
       $store->area=$request->get('area');
   ...
   //echo json_encode($request->all());
      ...
      // echo json_encode($data1);
      // exit();
    return view('Masters.edit_area',['edit_data'=>$data1,'data'=>$data,'area_all'=>$area_all]);
    }
